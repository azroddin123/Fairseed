    ...
    def get(self, request, *args, **kwargs):
        try:
            data = {
                ...
                "fund_raised": Campaign.objects.filter().aggregate(Sum("fund_raised"))[
                    "fund_raised__sum"
                ]
                or 0,
        ...
        try:
            ...
            fundraiser_data = (
                Campaign.objects.filter(
                    donors__created_on__range=(start_date, end_date)
                )
                .values("donors__created_on")
                .annotate(total_amount=Sum("donors__amount"))
            ...
            result = [
                {
                    ...
                    "total_amount": next(
                        (
                            ...
                            if item["donors__created_on"] == date.date()
                        ),
                        0,
                    ),
                }
...
class AdminCountryApi(APIView):
    def get(self, request, *args, **kwargs):
        user_data = [
            {"country": "India", "user_count": 4},
            {"country": "Australia", "user_count": 3},
        ]
        country_user_count = User.objects.values("country").annotate(
            user_count=Count("id")
        )
        return Response(
            {"error": False, "user_data": user_data}, status=status.HTTP_200_OK
        )
            ...
            with transaction.atomic():
                ...
                if request.data["approve_kyc"] == "true":
                    ...
                    return Response(
                        {
                            "error": False,
                            "data": "Bank Kyc  Update Request Approved Successfully",
                    ...
                    return Response(
                        {
                            "error": False,
                            "data": "Bank KYC Update Request Rejected Successfully",
                        },
                        status=status.HTTP_200_OK,
                    )
        except Exception as e:
...
class RevisionHistoryApi(APIView):
    def get(self, request, pk, *args, **kwargs):
        try:
            ...
            response = paginate_data(
                model=RevisionHistory,
                serializer=RHSerializer,
                request=request,
                data=data,
            )
        ...
        try:
            with transaction.atomic():
                ...
                campaign_serializer = CampaignSerializer(
                    object_instance, data=request_data, partial=True
                )
                if campaign_serializer.is_valid(raise_exception=True):
                    ...
                    documents_to_create = [
                        Documents(doc_file=item, campaign=campaign)
            ...
            with transaction.atomic():
                ...
                if request.data["approve_campaign"] == "true":
                    serializer = CampaignSerializer(
                        campaign, data=campaign.campaign_data, partial=True
                    )
                    ...
                    return Response(
                        {
                            "error": False,
                            "data": "Campaign Update Request Approved Successfully",
                        },
                    ...
                    return Response(
                        {
                            "error": False,
                            "data": "Campaign Update Request Rejected Successfully",
                        },
                        status=status.HTTP_202_ACCEPTED,
                    )
        except Exception as e:
    ...
    def get(self, request, pk=None, *args, **kwargs):
        try:
            ...
            return Response(
                {
                    "error": False,
                    "campaign_data": serializer.data,
                    "bank_data": serializer1.data,
                },
                status=status.HTTP_200_OK,
            )
        except Exception as e:
...
class CausEditApi(GenericMethodsMixin, APIView):
    ...
    def get(self, request, pk=None, *args, **kwargs):
        if pk is None:
            ...
            response = paginate_data(
                model=CauseEdit,
                serializer=CauseEditSerializer1,
                request=request,
                data=data,
            )
            return Response(response, status=status.HTTP_200_OK)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if request.data["approve_campaign"] == "true":
                    ...
                    serializer = CampaignSerializer(
                        campaign, data=cause_edit.campaign_data, partial=True
                    )
                    if serializer.is_valid(raise_exception=True):
                        serializer.save()
                    print("cause_edit", cause_edit.campaign_image)
                    if (
                        cause_edit.campaign_image is not None
                        and cause_edit.campaign_image != ""
                    ):
                        print("in if part")
                    ...
                    if docs:
                        ...
                        documents_to_create = [
                            Documents(doc_file=item, campaign=campaign) for item in docs
                        ]
                        Documents.objects.bulk_create(documents_to_create)
                    cause_edit.approval_status = "Approved"
                    cause_edit.save()
                    RevisionHistory.objects.create(
                        modified_by=request.thisUser,
                        campaign=campaign,
                        cause_data=cause_edit,
                    )
                    return Response(
                        {"error": True, "message": "Campaign Approved Successfully"},
                        status=status.HTTP_200_OK,
                    )
                else:
                    ...
                    return Response(
                        {
                            "error": True,
                            "message": "Campaign Rejected Successfully Successfully",
                        },
                        status=status.HTTP_200_OK,
                    )
        except Exception as e:
    ...
    def get(self, request, pk=None, *args, **kwargs):
        try:
            if pk is None:
                ...
                response = paginate_data(
                    BankKYCEdit, BankKYCEditSerializer, request=request, data=data
                )
                return Response(response, status=status.HTTP_200_OK)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if request.data["approve_kyc"] == "true":
                    ...
                    serializer = BankKYCEditSerializer(
                        bank_kyc, data=bank_edit.bank_data, partial=True
                    )
                    ...
                    bank_kyc.save()
                    return Response(
                        {"error": True, "message": "Kyc Request Approved Successfully"},
                        status=status.HTTP_200_OK,
                    )
                else:
                    ...
                    bank_edit.save()
                    return Response(
                        {"error": True, "message": "Kyc Request Rejected Successfully"},
                        status=status.HTTP_200_OK,
                    )
        except Exception as e:
            return Response(
...
class ReportedCauseAPI(GenericMethodsMixin, APIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        try:
            ...
            rc.delete()
            return Response(
                {"error": True, "message": "Campaign Status Rejected Successfully"},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response(
...
class ExportToCSV(APIView):
    def get(self, request, *args, **kwargs):
        ...
        for obj in data:
            row = [
                obj.title,
                obj.user.username if obj.user else None,
                obj.user.mobile_number if obj.user else None,
                obj.goal_amount,
                obj.fund_raised,
                obj.status,
                obj.end_date,
