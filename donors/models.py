...
class Donor(BaseModel):
    campaign = models.ForeignKey(
        "campaigns.Campaign", on_delete=models.CASCADE, related_name="donors"
    )
    ...
    status = models.CharField(
        max_length=124, choices=StatusChoices.choices, default=StatusChoices.PENDING
    )
...
def update_campaign(sender, instance, created, **kwargs):
    print("", created)
    print("donor post_save signal triggered")
...
class Withdrawal(BaseModel):
    ...
    withdrawal_status = models.CharField(
        max_length=124,
        choices=WithdrawalChoices.choices,
