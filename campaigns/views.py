...
from .models import Campaign, CampaignKycBenificiary
from .serializers import CampaignAccSerailaizer, CamapaignAccountSerializer
...
class ChangedCampaignsApi(APIView):
    def get(self, request):
        last_request_time = request.GET.get('last_request_time')
        if last_request_time:
            last_request_time = timezone.datetime.fromisoformat(last_request_time)
            changed_campaigns = Campaign.objects.filter(updated_at__gt=last_request_time)
        else:
            changed_campaigns = Campaign.objects.none()  # Return empty queryset initially
        
        serializer = CampaignSerializer(changed_campaigns, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class CampaignCatagoriesGetApi(APIView):  # camapign get API (Admin Panel)
    def get(self, request):
        ...
        serializer = CampaigncategorySerializer2(CampaignCategory, many = True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class CardAPIViewPagination(APIView):    
    def get(self, request):
        ...
        for c1 in paginated_campaigns:
            ...
            user_image = None
            if c1.user and hasattr(c1.user, 'user_image') and c1.user.user_image:
                user_image = c1.user.user_image.url
            api_data = {
                'id': c1.id,
                'title': c1.title,
                'user_name': user_data,
                ...
                'campaign_image': c1.campaign_image.url if c1.campaign_image else None,
...
class CampaignAnotherSearchApi(APIView):
    def get(self, request, *args, **kwargs):
        serializer = CampaignSearchSerializer(data=request.query_params.dict())
        serializer.is_valid(raise_exception=True)
        queryset = Campaign.objects.all()

        # Define a mapping of query parameters to database fields
        field_mapping = {
            'User': 'user__username',
            'Title': 'title',
            'Date': 'start_date',
        ...
        # Iterate through validated data and construct search parameters
        for key, value in serializer.validated_data.items():
            if value:
                field_name = field_mapping.get(key)
                if field_name:
                    if key == 'User':
                        # For exact match on username
                        search_params[field_name] = value
                    else:
                        # For case-insensitive search on other fields
                        search_params[f'{field_name}__icontains'] = value
...
class CampaignSearchApi(APIView):
    def get(self, request, *args, **kwargs):       
            ...
            if start_date and end_date:
                ...
                queryset = Campaign.objects.filter(start_date__gte=start_date, end_date__lte=end_date)
            if goal_amount:
                queryset = queryset.filter(goal_amount__icontains=goal_amount)
            if fund_raised:
                queryset = queryset.filter(fund_raised__icontains=fund_raised)
            ...
            if id:
                queryset = queryset.filter(user_id__icontains=id)
...
class AdminPanelCampaignView(APIView):
    def put(self, request, campaign_id):
        campaign = get_object_or_404(Campaign, id=campaign_id)
        serializer = CampaignAdminPSerializer(campaign, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
class CampaignRevisionApi(APIView):
    def get(self, request, pk):
...
class ReportedCampaignApi(APIView):
    def get(self, request):
        camp = Campaign.objects.filter(is_reported=True)
        serializer= ReportedCampaignSerilaizer(camp, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    def post(self, request):
        serializer = CampaignSerializer1(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
### Admin Panel > Withdrawl 
...
class CreateCampaignStep1(APIView):
    def get(self,request):
        camp = Campaign.objects.all()
...
class CreateCampaignStep2(APIView): 
    def get(self,request):
        camp = Campaign.objects.all()
...
# class CreateCampaignStep3(APIView):
    # def get(self,request):
...
class CreateCampaignStep3(APIView):
    ...
    def put(self, request, campaign_id):
        ...
        kyc_data = request.data.get('kyc', {})
        
        # Validate and save Campaign data
        campaign_serializer = CampaignSerializer(instance=campaign, data=campaign_data, partial=True)
        ...
        # Validate and save CampaignKycBenificiary data
        campaign_kyc, _ = CampaignKycBenificiary.objects.get_or_create(campaign=campaign)
        kyc_serializer = CampaignAccSerailaizer(instance=campaign_kyc, data=kyc_data)
        if kyc_serializer.is_valid():
            kyc_serializer.save()
            return Response(kyc_serializer.data, status=status.HTTP_200_OK)
        else:
...
class CampaignDetailStoryApi(APIView):
    ...
    def put(self, request,pk):
        campaign = get_object_or_404(Campaign, id=pk)
        serializer = CampaignDetailStorySerializer(campaign, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        # return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class RelegiousEducationCampApi(APIView):    
    def get(self, request):
        ...
        if expiring_soon and expiring_soon.lower() == 'true':
            campaigns = campaigns.filter(end_date__gte=ten_days_ago)
        # Newly Added Filter
        newly_added = params.get("newly_added")
        if newly_added and newly_added.lower() == 'true':
            campaigns = campaigns.filter(created_on__gte=ten_days_ago)

        # Common logic for 90% goal amount filter
        campaigns = campaigns.annotate(
            goal_amount_90_percent=F('goal_amount') * Decimal('0.9'),
        ...
        for c1 in paginated_campaigns:
            ...
            if c1.user and hasattr(c1.user, 'user_images') and c1.user.user_images:
                ...
                print(f"user_images: {user_images}")
...
class AdminCampaignKYCApi(APIView):
    def get(self,request):
        camp = CampaignKycBenificiary.objects.all()
        serializer = CamapaignKYCAPSerializer(camp, many = True)
        return Response(serializer.data , status=status.HTTP_200_OK)   
    ...
    def patch(self, request):
        ...
        serializer = CamapaignKYCViewSerializer(instance, data=request.data, partial = True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request):
        ...
        serializer = CamapaignKYCViewSerializer(instance, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
class AdminCampaignViewBankKYCApi(APIView):
    def get(self,request):
        camp = CampaignKycBenificiary.objects.all()
        serializer = CamapaignViewKYCViewSerializer(camp, many = True)
        return Response(serializer.data , status=status.HTTP_200_OK)

class VerifyKYCAPI(APIView):
    def put(self, request, pk):
        try:
            beneficiary = CampaignKycBenificiary.objects.get(pk=pk)
...
class AdminPCampaignKYCEditApi(APIView):
    def put(self, request,pk):
        try:
        ...
        serializer = CamapaignAdminPKYCSerializer(instance, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
