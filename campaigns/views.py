        ...
        try:
            if pk:
                ...
                data = Campaign.objects.filter(
                    status="Active", end_date__gt=current_date
                )
                response = paginate_data(
                    model=Campaign,
                    serializer=CampaignAdminSerializer,
                    request=request,
                    data=data,
                )
            ...
            return Response(
                {"error": True, "message": str(e)}, status=status.HTTP_200_OK
            )
...
class CampaigncategoryApi(GenericMethodsMixin, APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            response = paginate_data(
                Campaigncategory, CampaignCategorySerializer, request, data
            )
            return Response(response, status=status.HTTP_200_OK)
        except Exception as e:
...
class CampaignFilterApi(APIView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            campaign_data = Campaign.objects.filter(
                category=campaign_id, status="Active"
            )
            response = paginate_data(
                model=Campaign,
                serializer=CampaignSerializer2,
                request=request,
                data=campaign_data,
            )
            return Response(response, status=status.HTTP_200_OK)
...
class FeaturedCauseApi(APIView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            about_images = ["/media/about/Fairseed1.png", "/media/about/Fairseed2.png"]
...
class CampaignSearchAPIView(APIView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            serializer = CampaignSerializer(queryset, many=True)
            return Response(
                {"error": False, "rows": serializer.data}, status=status.HTTP_200_OK
            )
        except Exception as e:
...
class CampaignByCategoryApi(APIView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            response = paginate_model_data(
                model=Campaign,
                serializer=CampaignSerializer2,
                request=request,
                filter_key="category",
            )
            category_data = CampaignCategorySerializer(
                Campaigncategory.objects.get(id=cat_id)
            ).data
    ...
    def get(self, request, *args, **kwargs):
        try:
            data = {
                ...
                "total_donation": Campaign.objects.aggregate(Sum("fund_raised"))[
                    "fund_raised__sum"
                ]
                or 0,
                "donor_count": Donor.objects.filter(status="Approved").count(),
                "successful_campaign": Campaign.objects.filter(
                    is_successful=True
                ).count(),
                # this should be done when the amount is credited to student account.
                "student_benefited": Campaign.objects.filter(
                    is_withdrawal=True
                ).count(),
            }
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if filter_key == "most_supported":
                data = (
                    Campaign.objects.annotate(donor_count=Count("donors"))
                    .order_by("-donor_count")
        ...
        try:
            ...
            if filter_key == "most_supported":
                data = (
                    Campaign.objects.annotate(donor_count=Count("donors"))
                    .order_by("-donor_count")
                    .filter(status="Active", category=cat_id)
                )
            elif filter_key == "needs_love":
                data = (
                    Campaign.objects.annotate(donor_count=Count("donors"))
                ...
                data = Campaign.objects.filter(
                    status="Completed", category=cat_id
                ).order_by("created_on")
            else:
